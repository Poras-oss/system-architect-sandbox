...
import {
  ...
  Grid3X3, AlertTriangle, AlertCircle, Info, X,
} from "lucide-react";
...
export default function Navbar() {
  ...
  const handleExportPng = useCallback(async () => {
    ...
    if (!el) return;
    try {
      const dataUrl = await toPng(el, { backgroundColor: "#0c0c0e" });
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (data.nodes && data.edges) {
          loadState(data.nodes, data.edges);
        }
      } catch {
    ...
    <>
      <div className="h-11 flex items-center justify-between px-3 bg-surface-1 border-b border-border flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] text-muted-foreground hover:text-foreground hover:bg-surface-2 transition-colors"
          >
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] transition-colors ${snapToGrid ? "text-primary bg-primary/10" : "text-muted-foreground hover:text-foreground hover:bg-surface-2"}`}
          >
            <Grid3X3 size={13} />
            Snap
          </button>
      ...
      <div className="fixed bottom-0 left-[240px] right-0 z-30 h-7 bg-surface-1 border-t border-border flex items-center px-3 gap-3">
        <button onClick={handleShowIssues} className="flex items-center gap-2 text-[11px] text-muted-foreground hover:text-foreground transition-colors">
          ...
          {warnCount > 0 && (
            <span className="flex items-center gap-1 text-cat-data">
              <AlertTriangle size={11} /> {warnCount}
            </span>
          )}
          {errorCount === 0 && warnCount === 0 && (
            <span className="flex items-center gap-1">
              <Info size={11} /> No issues
            </span>
          )}
          <span>Issues</span>
        </button>
          ...
          {issues.map((issue) => (
            <div key={issue.id} className="px-3 py-1.5 flex items-start gap-2 text-[11px] border-b border-border/50">
              {issue.severity === "error" ? (
                ...
                <AlertTriangle size={12} className="text-cat-data flex-shrink-0 mt-0.5" />
              ) : (
      ...
      {showTemplates && (
        <div className="fixed inset-0 z-50 flex items-center justify-center" onClick={() => setShowTemplates(false)}>
          ...
          <div
            className="relative bg-surface-2 border border-border rounded-lg p-5 w-[520px] max-h-[80vh] overflow-y-auto"
            ...
            <div className="grid grid-cols-2 gap-3">
              {templates.map((tpl, idx) => (
                <button
                  ...
                  className="text-left p-3 rounded-lg bg-surface-1 border border-border hover:border-primary/50 hover:bg-surface-2 transition-all active:scale-[0.98]"
                >
...
function NavBtn({ icon: Icon, label, onClick }: { icon: any; label: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] text-muted-foreground hover:text-foreground hover:bg-surface-2 transition-colors active:scale-[0.97]"
    >
      <Icon size={13} />
      {label}
    </button>
